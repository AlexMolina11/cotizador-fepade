...
class Participantes{
    ...
    public function insertar($iddiscapacidad,$idmunicipio,$idnvomunicipio,$idinstitucion,$idcomunidad,$codigopar,$primernombre,$segundosnombre,$pirmerapellido,$segundosapellido,$fecha,$sexopar,$edadpar,$ocupacionpar,$concentimeintoparafotopar,$estudioactualpar,$escuelapar,$seccion,$turno,$profesor,$nombreresponsablepar,$telresponsablepar,$especialidadpar,$corredorpar,$niveleducativopar,$director,$tipoparticiparnte,$usuario,$ip,$iddepartamento,$idcorredor){ //Se agregó departamento
		
        $usuario      =$_SESSION["login"];
        $ip           =$_SERVER["REMOTE_ADDR"];					  
		
		$sql = "INSERT INTO participantes(IDDISCAPACIDAD, IDMUNICIPIO, IDNVOMUN, IDINSTITUCION, IDCOMUNIDAD, IDTIPOPARTICIPANTE, CODIGOPAR, PRIMERNOMBRE, SEGUNDOSNOMBRE, PRIMERAPELLIDO, SEGUNDOSAPELLIDO, FECHANACPAR, SEXOPAR, EDADPAR, OCUPACIONPAR, CONCENTIMEINTOPARAFOTOPAR, ESTUDIOACTUALPAR, ESCUELAPAR, SECCION, TURNO, PROFESOR, NOMBRERESPONSABLEPAR, TELRESPONSABLEPAR, ESPECIALIDADPAR, CORREDORPAR, NIVELEDUCATIVOPAR, DIRECTOR, USUREGPAR, IPREGPAR, FECHAREGPAR, ESTADOPAR, IDCORREDOR, IDDEPARTAMENTO, FECHAMODPAR, USUMODPAR) 
        VALUES (
            $iddiscapacidad, $idmunicipio, $idnvomunicipio, $idinstitucion, $idcomunidad, $tipoparticiparnte, '$codigopar', 
            UPPER(TRIM('$primernombre')), UPPER(TRIM('$segundosnombre')), UPPER(TRIM('$pirmerapellido')), UPPER(TRIM('$segundosapellido')), 
            '$fecha', '$sexopar', '$edadpar', '$ocupacionpar', '$concentimeintoparafotopar', '$estudioactualpar', 
            '$escuelapar', '$seccion', '$turno', '$profesor', '$nombreresponsablepar', '$telresponsablepar', 
            '$especialidadpar', '$corredorpar', '$niveleducativopar', '$director', '$usuario', '$ip', NOW(), '1', 
            $idcorredor, '$iddepartamento', NOW(), '$usuario'
        )";
        return ejecutarConsulta($sql);
    }
    
	//Implementamos un método para editar registros
    public function editar($idparticipante,$iddiscapacidad,$idmunicipio,$idnvomunicipio,$idinstitucion,$idcomunidad,$codigopar,$primernombre,$segundosnombre,$pirmerapellido,$segundosapellido,$fecha,$sexopar,$edadpar,$ocupacionpar,$concentimeintoparafotopar,$estudioactualpar,$escuelapar,$seccion,$turno,$profesor,$nombreresponsablepar,$telresponsablepar,$especialidadpar,$corredorpar,$niveleducativopar,$director,$tipoparticiparnte,$iddepartamento, $idcorredorpar){ //Se agregó departamento
        $usuario      =$_SESSION["login"];
		
        $sql = "UPDATE participantes 
                SET IDDISCAPACIDAD = $iddiscapacidad,
                    IDMUNICIPIO = $idmunicipio,
                    IDNVOMUN = $idnvomunicipio,
                    IDINSTITUCION = $idinstitucion,
                    IDCOMUNIDAD = $idcomunidad,
                    IDTIPOPARTICIPANTE = $tipoparticiparnte,
                    CODIGOPAR = '$codigopar',
                    PRIMERNOMBRE = UPPER(TRIM('$primernombre')),
                    SEGUNDOSNOMBRE = UPPER(TRIM('$segundosnombre')),
                    PRIMERAPELLIDO = UPPER(TRIM('$pirmerapellido')),
                    SEGUNDOSAPELLIDO = UPPER(TRIM('$segundosapellido')),
                    FECHANACPAR = '$fecha',
                    SEXOPAR = '$sexopar',
                    EDADPAR = $edadpar,
                    ...
                    CORREDORPAR = '$corredorpar',
                    NIVELEDUCATIVOPAR = '$niveleducativopar',
                    DIRECTOR = '$director',
                    IDCORREDOR = $idcorredorpar,
    ...
    public function mostrar($idparticipante){
        $sql="SELECT `participantes`.`IDPARTICIPANTE`, 
                    ...
                    `IDDEPARTAMENTO`, /*Se agregó departamento*/
                    `participantegrupo`.`IDGRUPO`,
                    `asistencias`.`PRIMERAVEZASI` AS 'PRIMERAVEZ',
                    `asistencias`.`FECHAASI` AS 'FECHAPRIMERAVEZ'
            FROM `participantes` 
            LEFT OUTER JOIN `participantegrupo` ON `participantegrupo`.`IDPARTICIPANTE` = `participantes`.`IDPARTICIPANTE`
            LEFT OUTER join `asistencias` ON `participantes`.`IDPARTICIPANTE` = `asistencias`.`IDPARTICIPANTE`
    ...
    public function selectCorredor(){ 
        $sql="SELECT NOMBRECOR FROM corredor WHERE ESTADOCOR=1 ORDER BY `NOMBRECOR` ASC";
        return ejecutarConsulta($sql);
    }

    public function selectCorr($iddepartamento){ 
        $sql="SELECT  NOMBRECOR FROM corredor c
            WHERE c.IDDEPARTAMENTO=$iddepartamento AND c.ESTADOCOR=1 ORDER BY `NOMBRECOR` ASC";
        return ejecutarConsulta($sql);
    }

    public function selectCorredor_mostrar($iddepartamento,$idcorredorpar){
        $sql="SELECT NOMBRECOR FROM corredor c WHERE c.IDDEPARTAMENTO=$iddepartamento AND c.IDCORREDOR=$idcorredorpar AND c.ESTADOCOR=1 ORDER BY NOMBRECOR ASC";
        return ejecutarConsulta($sql);
    }
    ...
    public function selectInstituciones($idcorredorpar){
        ...
        WHERE b.NOMBRECOR='$idcorredorpar' AND ESTADOINS=1";
        return ejecutarConsulta($sql);
    }

    public function selectCentroEducativo_mostrar($idcorredorpar,$idinstitucion) {
        $sql="SELECT * FROM institucion i WHERE i.IDCORREDOR=$idcorredorpar AND i.IDINSTITUCION=$idinstitucion AND ESTADOINS=1 ORDER BY i.NOMBREINS ASC";
        return ejecutarConsulta($sql);
    }
