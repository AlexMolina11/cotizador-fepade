...
switch ($_GET["op"]) {
    case 'guardaryeditar':
        if (empty($idparticipante)) {
            //$rspta_consulta=$participantes->consulta_existencia($tipoparticiparnte,$idcomunidad,$iddepartamento,$idmunicipio,$idinstitucion,$primernombre,$segu,$pirmerapellido,$fechanacpar,$sexopar);
            ...
            if($rspta_consulta['IDPARTICIPANTE'] == NULL) {
                //$rspta=$participantes->insertar($iddiscapacidad,$idmunicipio,$idinstitucion,$idcomunidad,$codigopar,$primernombre,$segundosnombre,$pirmerapellido,$segundosapellido,$fecha,$sexopar,$edadpar,$ocupacionpar,$concentimeintoparafotopar,$estudioactualpar,$escuelapar,$seccion,$turno,$profesor,$nombreresponsablepar,$telresponsablepar,$especialidadpar,$corredorpar,$niveleducativopar,$director,$tipoparticiparnte,$usuario,$ip);
                $rspta=$participantes->insertar($iddiscapacidad,$idmunicipio,$idinstitucion,$idcomunidad,$codigopar,$primernombre,$segundosnombre,$pirmerapellido,$segundosapellido,$fecha,$sexopar,$edadpar,$ocupacionpar,$concentimeintoparafotopar,$estudioactualpar,$escuelapar,$seccion,$turno,$profesor,$nombreresponsablepar,$telresponsablepar,$especialidadpar,$corredorpar,$niveleducativopar,$director,$tipoparticiparnte,$usuario,$ip,$iddepartamento,$corredorpar); //Se agregó el departamento
                echo $rspta? "Participante Registrado":"Participante no se pudo registrar";
            } else {
                echo "Existente";
            }
        }else {
           //$rspta=$participantes->editar($idparticipante,$iddiscapacidad,$idmunicipio,$idinstitucion,$idcomunidad,$codigopar,$primernombre,$segundosnombre,$pirmerapellido,$segundosapellido, $fecha,$sexopar,$edadpar,$ocupacionpar,$concentimeintoparafotopar,$estudioactualpar,$escuelapar,$seccion,$turno,$profesor,$nombreresponsablepar,$telresponsablepar,$especialidadpar,$corredorpar,$niveleducativopar,$director,$tipoparticiparnte);
           $rspta=$participantes->editar($idparticipante,$iddiscapacidad,$idmunicipio,$idinstitucion,$idcomunidad,$codigopar,$primernombre,$segundosnombre,$pirmerapellido,$segundosapellido, $fecha,$sexopar,$edadpar,$ocupacionpar,$concentimeintoparafotopar,$estudioactualpar,$escuelapar,$seccion,$turno,$profesor,$nombreresponsablepar,$telresponsablepar,$especialidadpar,$corredorpar,$niveleducativopar,$director,$tipoparticiparnte,$iddepartamento,$corredorpar); //Se agregó el departamento
    ...
    case 'selectDepa':
        $rspta=$comunidad->selectDepa($idmunicipio);
		echo '<option value="">--Seleccione Departamento--</option>'; 
        while ($reg = $rspta->fetch_object()) {
            echo '<option value='.$reg->IDDEPARTAMENTO.' selected>'.$reg->NOMBREDEP.'</option>';
        }
	break;

    case 'selectDepartamento_mostrar':
        $rspta=$participantes->selectDepartamento_mostrar($iddepartamento);
		//echo '<option value="">--Seleccione Departamento--</option>'; 
        while ($reg = $rspta->fetch_object()) {
            echo '<option value='.$reg->IDDEPARTAMENTO.'>'.$reg->NOMBREDEP.'</option>';
        }
    ...
    case 'selectMunicipio':
        $rspta=$participantes->selectMunicipio();
    ...
    case 'selectMuni':
        $rspta=$comunidad->selectMuni($iddepartamento);
    ...
    case 'selectCorredor_mostrar':
        $rspta=$participantes->selectCorredor_mostrar($iddepartamento,$corredorpar);
		//echo '<option value="">--Seleccione Corredor--</option>'; 
        while ($reg = $rspta->fetch_object()) {
    ...
    case 'selectInstituciones':
        echo '<option value="">--Seleccione Centro Educativo--</option>';
        $rspta=$participantes->selectInstituciones($corredorpar);
        while ($reg = $rspta->fetch_object()) {
            echo '<option value='.$reg->IDINSTITUCION.'>'.$reg->NOMBREINS.'</option>';
        }
	break;
    ...
    case 'selectCentroEducativo_mostrar':
        $rspta=$participantes->selectCentroEducativo_mostrar($corredorpar,$idinstitucion);
		//echo '<option value="">--Seleccione Centro Educativo--</option>'; 
        while ($reg = $rspta->fetch_object()) {
